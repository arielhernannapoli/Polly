...
namespace Polly.Retry.Example.TypedClient.Controllers
{
    ...
    public class BackendController : ControllerBase
    {
        ...
        public IActionResult GetData()
        {
            ...
            if (_callCount % 3 != 0)
            {
                _logger.LogWarning($"[TypedClient] Backend: Simulando fallo en llamada {_callCount}");
                return StatusCode(500, "Error temporal en el servidor");
            }

            _logger.LogInformation($"[TypedClient] Backend: Respondiendo correctamente en llamada {_callCount}");
            return Ok(new 
            { 
                message = "Datos del backend (Typed HttpClient)",
