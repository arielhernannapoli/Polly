...
namespace Polly.Retry.Example.DelegatingHandler.Controllers
{
    ...
    public class ConsumerController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDataFromBackend()
        {
            try
            {
                _logger.LogInformation("Consumer: Iniciando llamada a Backend con Polly retry (DelegatingHandler)");
                var data = await _backendService.GetDataAsync();
                return Ok(new 
                { 
                    success = true,
                    data = data,
                    message = "Datos obtenidos del backend con reintentos de Polly (usando DelegatingHandler)"
                });
            }
